...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = (
            'id', 'avatar', 'is_subscribed', 'username', 'first_name',
            'last_name', 'email')
...
class IngredientInRecipeSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        data = super().to_representation(instance)
        for key, val in data.pop('ingredient').items():
            data[key] = val
        return data
...
class RecipeSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        invalid_data = filter(
            lambda ingredient: (not ingredient.get('id')
                                or not ingredient.get('amount')),
            ingredients)
        ...
        invalid_data = filter(
            lambda ingredient: ingredient.get('amount') < 1,
            ingredients)
        if list(invalid_data):
    ...
    class Meta:
        model = Recipe
        fields = (
            'id', 'tags', 'name', 'text', 'ingredients', 'image',
