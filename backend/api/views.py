...
from domain.models import (
    Ingredient, Recipe, RecipeIngredient, Subscription, Tag,
...
from .utils import (
    Pagination, RecipeFilterSet, SearchFilter, get_or_create_short_link)
...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Recipe.detailed.annotate_extra_info(
            self.request.user
        ).order_by('-pub_date')
    ...
    def download_shopping_cart(self, request, *args, **kwargs):
        ings = RecipeIngredient.objects.filter(
    ...
    def __handle_favorites_shopping_cart(self, request, pk, model_cls, target):
        ...
        _, created = model_cls.objects.get_or_create(user=request.user,
                                                     recipe=recipe)
        if not created:
...
class UserViewSet(mixins.CreateModelMixin,
    ...
    def avatar(self, request, *args, **kwargs):
        if request.method == 'DELETE':
            ...
            request.user.save()
            return Response(status=status.HTTP_204_NO_CONTENT)
        serializer = UserSetAvatarSerializer(data=request.data,
    ...
    def subscribe(self, request, *args, **kwargs):
        ...
        if request.method == 'DELETE':
            try:
                following = Subscription.objects.get(user=request.user,
                                                     following=following)
                following.delete()
                return Response(status=status.HTTP_204_NO_CONTENT)
